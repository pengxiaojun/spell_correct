...
def edits1(word):
    n = len(word)
    return set([word[0:i]+word[i+1:] for i in range(n)]+                    #deletetion
            [word[0:i]+word[i+1]+word[i]+word[i+2:] for i in range(n-1)]+   #transpostion
...
#############################################spell test#######################################
def spell_test(tests, bias=None, verbose=False):
    import time
    n, bad, unknow, start = 0, 0, 0, time.clock()
    if (bias):
        for target in test:NWORDS[target] += bias
    for target, wrongs in tests.items():
        for wrong in wrongs.split():
            ...
            if (w != target):
                bad += 1
                unknow += (target not in NWORDS)
                if (verbose):
                    print ("correct(%s) => %s (%d); expected %s (%d)" % (wrong, w, NWORDS[w], target, NWORDS[target]))
    return dict(bad=bad, n=n, bias=bias, pct=int(100. - 100.*bad/n), unknow=unknow, sesc=int(time.clock() - start))
...
if __name__ == '__main__':
    print(spell_test(tests1,verbose=True))
